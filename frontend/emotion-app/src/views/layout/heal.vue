  ...
  <div id="heal">
    ...
    <div class="bg">
      <div class="components wrapper">
        <ul>
          <li v-for="(item, index) in healList" :key="item.id"><heal-item :id="index" :healList="healList"></heal-item></li>
        </ul>
      </div>
...
<script>
import healItem from '@/components/healItem.vue';
export default {
    name: 'healSpace',
    components: {
      healItem
    ...
    data () {
      return {
        healList: [
          ...
          { id: 2, bgUrl: require('@/assets/image/疗愈空间/情绪拼图背景.png'), url: require('@/assets/image/疗愈空间/情绪拼图.png'), title: '情绪拼图', introduce: '把杂乱的情绪慢慢变完整', toUrl: '/emotionalPuzzle' },
...
.schedule {
  ...
  height: 100px;
  border-radius: 20px;
  background-image: url('@/assets/image/疗愈空间/疗愈进度背景.png');
  ...
  .continue {
    ...
    border-radius: 20px;
    background: #ffc894;
    box-shadow: 0 2px 4px 0 #ab814f80;
    color: #261700cc;
    font-size: 16px;
    font-weight: 600;
    line-height: 40px;
